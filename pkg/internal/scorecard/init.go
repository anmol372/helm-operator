...
import (
	...
	"github.com/joelanford/helm-operator/pkg/internal/util/kustomize"
)
...
const (
	...
	//TODO: will it need bundle/scorecard
...
func generate(testImageTag, outputDir string) error {
	...
	relBasePath := filepath.Join("bases", ConfigFileName)
	basePath := filepath.Join(basesDir, ConfigFileName)
	...
	basicPatchFileName := fmt.Sprintf("basic.%s", ConfigFileName)
	...
	olmPatchFileName := fmt.Sprintf("olm.%s", ConfigFileName)
